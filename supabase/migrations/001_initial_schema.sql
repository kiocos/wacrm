...
-- UUID defaults use gen_random_uuid(), built into Postgres 13+.
-- Deliberately NOT uuid-ossp: on hosted Supabase that extension is
-- pre-installed in the `extensions` schema, so a CREATE EXTENSION
-- IF NOT EXISTS here no-ops and unqualified uuid_generate_v4() then
-- fails under the migration session's search_path — a fresh
-- `supabase db push` died on this file's first CREATE TABLE.
